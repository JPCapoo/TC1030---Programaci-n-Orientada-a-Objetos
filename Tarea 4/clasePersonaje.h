...
#include "claseLinterna.h"

using namespace std;

class Personaje : public Linterna {
    private: //Atributos
        int nivel;
        int correr;
        int saltar;
    public: //Métodos
        ...
        Personaje(string, int, int, int, int); //Constructor
        virtual void shoot();
        int setCorrer(int);
        int setSaltar(int);
        void muestraDatosP();
...
Personaje::Personaje(string nI, int nL, int n, int c, int s): Linterna(nI, nL){
    nivel = n;
    correr = c;
    saltar = s;
};
